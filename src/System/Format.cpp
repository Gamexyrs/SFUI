...
namespace sf::ui {
  ...
  inline func Format::getPosition(const Rect<Type>& rect) -> Vector2<Type> {
  ...
  inline func Format::getSize(const Shape& target) -> Vector2f {
    return Format::getSize(target.getGlobalBounds());
  }
  inline func Format::getSize(const Text& target) -> Vector2f {
    return Format::getSize(target.getGlobalBounds());
  }
  
  inline func Format::getFPS(const Clock& clock) -> String {
    return String(std::to_wstring((static_cast<unsigned>(std::ceil(1 / clock.getElapsedTime().asSeconds())))));
  }
  
  template<typename Type>
  inline func Format::reverse(const Vector2<Type>& vector2) -> Vector2<Type> {
    return{vector2.y, vector2.x};
  }
  
  inline func Format::toString(const VideoMode& videoMode) -> String {
  ...
  template<typename Type>
  inline func Format::toString(const Rect<Type>& rect) -> String {
  ...
  template<typename Type>
  inline func Format::toString(const Vector2<Type>& vector2) -> String {
    return String(L"[x:" + std::to_wstring(vector2.x) + L", y:" + std::to_wstring(vector2.y) + L"]");
  }
  template<typename Type>
  inline func Format::toString(const Vector3<Type>& vector3) -> String {
